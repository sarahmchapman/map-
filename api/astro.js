...
const S = 1e-8;
function vs(terms, tau) { return terms.reduce((s, v) => s + v[0] * Math.cos(v[1] + v[2] * tau), 0); }
...
function geoLon(p, e) { return n360(Math.atan2(p.y - e.y, p.x - e.x) * 180 / Math.PI); }
...
function kepler(M, e) {
  let E = M;
  for (let i = 0; i < 10; i++) { const d = E - e * Math.sin(E) - M; E -= d / (1 - e * Math.cos(E)); if (Math.abs(d) < 1e-10) break; }
  return E;
}
function kepGeo(L0r,Lr,e0r,er,w0r,wr,i0r,ir,O0r,Or,ar,jd,ex,ey) {
  const T=(jd-2451545)/36525,L=n360(L0r+Lr*T),e=e0r+er*T,w=n360(w0r+wr*T),i=R(i0r+ir*T),O=R(O0r+Or*T),M=R(n360(L-w)),E=kepler(M,e),nu=2*Math.atan2(Math.sqrt(1+e)*Math.sin(E/2),Math.sqrt(1-e)*Math.cos(E/2)),r=ar*(1-e*Math.cos(E));
  const sm=R(w)-O,u=nu+sm,x=r*(Math.cos(O)*Math.cos(u)-Math.sin(O)*Math.sin(u)*Math.cos(i)),y=r*(Math.sin(O)*Math.cos(u)+Math.cos(O)*Math.sin(u)*Math.cos(i));
  return n360(Math.atan2(y-ey,x-ex)*180/Math.PI);
}
function sunLon(jd) {
  const T=(jd-2451545)/36525,L0=n360(280.46646+36000.76983*T),M=n360(357.52911+35999.05029*T-.0001537*T*T),Mr=R(M);
  return n360(L0+(1.914602-.004817*T-.000014*T*T)*Math.sin(Mr)+(0.019993-.000101*T)*Math.sin(2*Mr)+.000289*Math.sin(3*Mr));
}
function moonLon(jd) {
  const T=(jd-2451545)/36525,L0=n360(218.3165+481267.8813*T),M=n360(357.5291+35999.0503*T),Mp=n360(134.9634+477198.8676*T),D=n360(297.8502+445267.1115*T),F=n360(93.2721+483202.0175*T);
...
function lonToRaDec(lon) {
  const l=R(lon),e=R(OBL);
  return { ra: n360(Math.atan2(Math.sin(l)*Math.cos(e),Math.cos(l))*180/Math.PI), dec: Math.asin(Math.sin(e)*Math.sin(l))*180/Math.PI };
}
...
function buildACG(jd) {
  ...
  for (const [name, lon] of Object.entries(lons)) {
    ...
    const mc = [], ic = [], asc = [], dsc = [];
    for (let lat = -85; lat <= 85; lat += 1) { mc.push([lat, mcLon]); ic.push([lat, icLon]); }
